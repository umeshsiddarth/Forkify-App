...
function createRecipeObject(data) {
  ...
  return {
    ...
    ...(recipe.key && { key: recipe.key }), // If key exists then key: recipe.key will be added to this object. We implemented short circuting and spread operator on the object.
  };
}
...
export const uploadRecipe = async function (newRecipe) {
  try {
    const ingredients = Object.entries(newRecipe)
      ...
      .map((ing) => {
        // The replaceAll method removes all the spaces even if it is in an ingredient.
        // const ingArr = ing[1].replaceAll(" ", "").split(",");

        const ingArr = ing[1].split(",").map((el) => el.trim());
        if (ingArr.length !== 3)
          throw new Error(
            "Wrong ingredient format. Please check and try again!"
          );
